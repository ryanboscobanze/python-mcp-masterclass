"""MCP completion server example (stdio transport).

Demonstrates the @mcp.completion() decorator for argument autocompletion:

- resource template: github://repos/{owner}/{repo}
  - completing 'repo' with context (owner="modelcontextprotocol") returns known repos

- prompt: review_code(language, code)
  - completing 'language' returns matching languages from a fixed list

Run with:
    python 2c_completion/server_completion.py
"""
...
mcp = FastMCP(name="CompletionExample")
